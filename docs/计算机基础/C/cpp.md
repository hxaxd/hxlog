...
3. C语言的的编译链接以及硬件层面的细节，在汇编语言，计算机系统基  础，计算机组成原理与体系结构，编译原理这些内容中提及，但也许相当具体的内容需要了解编译器实现  
...
1. sizeof类型必须加() 返回类型是lu/llu（比较时注意类型）
2. printf修饰符 #（完全展示）
3. c会将连续字符串融合
...
3. 不可重定义同名宏
4. 类函数宏注意替换后的运算优先级
5. `#x`可以在替换体中的字符串中使用x这一个宏参数的字符串形式
6. `##`可以将记号粘合
...
16. assert(bool)若为假则打印err，停止程序 _Static_assert(bool，char*)若为假则打印字符串，无法编译
