...
力扣no.117  
填充每个节点的下一个右侧节点指针 II(node中增加next指针域,缓存层序遍历)  
```C++
class Solution {
public:
...
```
数学方法,其实取余9就好了,但是不同的语言有不同的取余规则

力扣no.1863  
找出所有子集的异或总和再求和  
```C++
// 枚举子集,用一个n位的二进制数的每一位表示子集的元素,1表示该元素在子集中,0表示不在子集中
...
```
所有元素该位都为0,所有子集异或总和中该位均为,0,反之,所有子集异或总和中该位为0的个数与为1的个数相等,均为2^(n-1)  
